...
void CCEGLViewQt::handleTouchesCancel(
	int num, int ids[], float xs[],
	float ys[])
{
	CCSet set;
...
void CCEGLViewQt::FillTouchesEndOrCancel(
	CCSet &set, int num, int ids[],
	float xs[], float ys[])
{
	for (int i = 0; i < num; i++)
	{
...
{
	touch->setTouchInfo(
		id, (x - m_obViewPortRect.origin.x) / m_fScaleX,
